...
const NewStory = () => {
  ...
  const handleCreate = async () => {
    ...
    router.push(`/generating?story=${story.id}`);
  };
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-5">
        <div className="text-center space-y-6 max-w-sm">
          ...
          <Button className="rounded-full px-8" onClick={() => router.push("/onboarding?new=true")}>
            <Plus size={16} className="mr-2" /> Create a Profile
          </Button>
          <div>
            <Link href="/library" className="text-sm text-muted-foreground hover:text-foreground underline">
              Back to Library
            </Link>
          </div>
        </div>
